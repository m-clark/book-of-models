...
A *matrix* is a 2 dimension collection of vectors.

Before we show you any code, it will be best to show you an actual matrix. 
...
If you think about most tables you've ever seen, you'll see that the simple matrix looks just like them!

This is matrix has 2 dimensions, rows and columns. When we talk about the dimensions of a matrix, we always make note of the rows first, followed by the columns. We have 2 rows and 3 columns therefore, we have a $2x3$ matrix.

## Addition

Matrix addition, along with subtraction, are the easiest concepts when dealing with matrices. While they are easy to grasp, you will not find them featured as prominently as matrix multiplication.
...
You probably notices that we gave each scalar within the matrix a label associated with its row and column position. We can use these to see how we will produce the new matrix: 
...
Let's see those two matrices again:
...
Now, you probably have some confidence in doing matrix operations. Just as quickly as we built that confidence, it will be crushed when dealing with matrix multiplication. 
...
Matrix A has dimensions of $2x3$, as does Matrix B. Putting those side by side -- $2x3 * 2x3$ -- we see that our inner dimensions do not match. 
