...
![Likelihood function for one parameter.](img/estim-likelihood_one_param.svg){#fig-r-likelihood-plot width=100%}
:::
...
It turns out that given a fixed $\lambda$ penalty ridge regression estimates can be derived analytically. @clark_model_2021 (one of your authors) has an example if interested in his [Model Estimation by Example](https://m-clark.github.io/models-by-example/penalized-maximum-likelihood.html#l2-ridge-regularization) demos.
:::
...
:::{.content-visible when-format='pdf'}
![Stochastic gradient descent path.](img/estim-sgd.svg){#fig-r-stochastic-gradient-descent width=57%}
:::
