...
![](img/chapter_gp_plots/gp_plot_3.svg){width=75% fig-align='center'}
...
![World happiness data summary.](img/happiness_data_summary.png){#fig-r-happiness-data width=66%}
:::
...
For our purposes here, we'll drop any rows with missing values, and we'll use scaled features so that they have the same variance, which, as noted in the data chapter (@sec-data), can help make estimation easier.  
...
Now let's look at our MSE, and we'll also inspect the square root of it, or the **Root Mean Squared Error** (\index{metrics!root mean squared error}), as that puts things back on the original target scale and tells us the standard deviation of our prediction errors. We also add the **Mean Absolute Error** (MAE)\index{metrics!mean absolute error} as another metric with straightforward interpretation. 
...
Inspecting the metrics, we can see that we are off on average by over a point for model A (MAE), and a little over half a point on average for model B. So we can see that model B is not only better, but it results in a `r mse_tab[['RMSE % drop']][2]` drop in RMSE, and similar for MAE. We'd definitely prefer it over model A. This approach is also how we can compare models in a general fashion.
...
#| tbl-cap: Comparison of OLS Results to a Standard Function
...
#| tbl-cap: Comparison of Max. Likelihood Results to a Standard Function
...
our_result_tbl = tibble(
    ...
    # though a long running GH issue, none of the following does anything with table font size
    # for pdf; works great for html, or viewer so it's not the code
    tab_options(
        # table.font.size = pct(5),
        table.font.size = 'scriptsize',
        # footnotes.font.size = pct(33),
    ) |> 
    tab_style(
        style = cell_text(size = 'scriptsize'),
        locations = cells_body(columns = c('Standard', 'Our Result'))
    ) #|>
    # tab_style(
...
#| echo: false
#| eval: false
#| label: fig-r-likelihood-plot-3d-compute
...
# need estimates for path plot later
save(estimates, p, file = 'img/estim-likelihood_surface.RData')
...
load('img/estim-likelihood_surface.RData')
...
::: {.content-visible when-format='pdf'}
![Likelihood surface for two parameters.](img/estim-likelihood_surface.svg){#fig-r-likelihood-plot-3d width=66%}
:::
...
:::{.callout-note title='MLE and OLS' collapse='true'}
...
\index{penalized objectives|(}
One thing we may want to take into account with our models is their complexity, especially in the context of **overfitting**\index{overfitting}. We talk about this with machine learning also (@sec-ml-core-concepts), but the basic idea is that we can get too familiar with the data we have, and when we try to predict on new data the model hasn't seen before, model performance suffers. In other words, we are not generalizing well (@sec-ml-generalization). 

One way to deal with this is to penalize the objective function value for complexity, or at least favor simpler models that might do as well. In some contexts this is called **regularization**\index{regularization}, and in other contexts **shrinkage**\index{shrinkage}, since the parameter estimates are typically shrunk toward some specific value (e.g., zero).

As a starting point, in our basic linear model we can add a penalty that is applied to the size of  coefficients.  This is called **ridge regression**\index{ridge (L2) penalty}, or, more mathily, **L2 regularization**\index{regularization!L2}\index{regularization!ridge}. We can write this formally as:
...
In the end this is just a small change to OLS regression (@eq-ols), but it can make a big difference. It introduces some bias in the coefficients -- recall that OLS is unbiased if assumptions are met -- but it can help to reduce variance, which can help the model perform better on new data (@sec-ml-generalization-understanding). In other words, we are willing to accept some bias in order to get a model that generalizes better.
...
We can compare this to built-in functions as we have before[^rpyridge], and we can see that the results are very similar, but not exactly the same.  We would not worry about such differences in practice, but the main point is again, that we can use simple functions that do just about as well as those from common packages.
...
our_ridge_tbl = tibble(
    ...
    gt(decimals = 2) |>
    gt::tab_footnote(
        footnote = 'Showing results from the R glmnet package with alpha = 0, lambda = .1.',
...
It turns out that given a fixed $\lambda$ penalty ridge regression estimates can be derived analytically. @clark_model_2021 (one of your authors) has an example in his [Model Estimation by Example](https://m-clark.github.io/models-by-example/penalized-maximum-likelihood.html#l2-ridge-regularization) demos.
:::
...
\index{optimization!for classification|(}
...
\index{metrics!log loss}
A very common objective function for classification is **log loss**, sometimes called logistic loss, or **cross-entropy**[^LLvsCE]. For a binary target, it is:

[^LLvsCE]: A nice demo from the [pytorch]{.pack} perspective can be found at @raschka_losses_2022.
...
\index{optimization!algorithms!common}
...
The size of the 'step' is called the **learning rate**\index{learning rate}\index{step size|see{learning rate}}. Like the penalty parameter we saw with penalized regression, it is a *hyperparameter* that we can tune through cross-validation (@sec-ml-tuning). If the learning rate is too small, it will take a longer time to converge. If it's too large, we might overshoot the objective and miss the best parameters. There are a number of variations on gradient descent that have been developed over time. Let's see this in action with the world happiness model.
...
```{r}
#| label: r-gradient-descent-apply
X = df_happiness |>
    select(life_exp_sc:gdp_pc_sc) |>
    as.matrix()
...
```{python}
#| label: py-gradient-descent-apply
our_gd = gradient_descent(
...
We compare our results in @tbl-gradient-descent. As usual, we see that the results are very similar to the standard linear regression approach. Once again, we have demystified a step in the modeling process!
...
In addition, when we visualize the loss function across iterations, we see a smooth decline in the MSE value as we go along each iteration (@fig-r-gradient-descent). This is a good sign that we are converging to an optimal solution.
...
:::{.content-visible when-format='pdf'}
![Loss with gradient descent.](img/estim-gradient_descent.svg){#fig-r-gradient-descent width=66%}
:::
\index{gradient descent|)}
...
Let's now use the functions as we did before. We'll show the results in @tbl-stochastic-gradient-descent.

:::{.panel-tabset}
##### R

```{r}
#| label: r-stochastic-gradient-descent-apply
#| results: 'hide'
X = df_happiness |>
...
```{python}
#| label: py-stochastic-gradient-descent-apply
#| results: 'hide'
X = df_happiness[['life_exp_sc', 'corrupt_sc', 'gdp_pc_sc']].to_numpy()
y = df_happiness['happiness'].to_numpy()
...
Now we'll compare it to OLS estimates and our previous 'batch' gradient descent results.  Even though SGD normally would not be used for such a small dataset, we at least get close[^closebutnosgd]!

[^closebutnosgd]: You'd get better results in a couple of ways. The easiest is just to repeat the process a couple of times and average the results. This is a common approach in SGD. The initial shuffling that we did can help with convergence as well, and it would be done each repetition. With larger data and repeated runs/epochs, shuffling allows the samples/batches to be more representative of the entire dataset. Also, we 'hand-tune' our learning rate and step size here, but normally we'd use cross-validation to find the best values.
...
@fig-r-stochastic-gradient-descent shows the estimates as they moved along the data. For this plot we don't include the intercept, as it's on a notably different scale. We can see that the estimates are moving around a bit, but they appear to be converging to a solution. 
...
:::{.content-visible when-format='html'}
![Stochastic gradient descent path.](img/sgd.gif){#fig-r-stochastic-gradient-descent}
:::

:::{.content-visible when-format='pdf'}
![Stochastic gradient descent path.](img/estim-sgd.svg){#fig-r-stochastic-gradient-descent width=75%}
:::
