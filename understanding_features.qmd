...
\index{linear model!parameters|(}
We saw in the foundations chapter chapter ([-@sec-lm-interpretation-feature]) that we can get a lot out of the basic output from standard linear models like linear regression. Our starting point should be the coefficients or weights, which can give us a sense of the direction and magnitude of the relationship between the feature and the target given their respective scales. We can also look at the standard errors and confidence intervals\index{confidence interval} to get a sense of the uncertainty in those estimates. @tbl-r-regression-coefs is a basic summary of the coefficients for our regression model on the training data.
...
\index{features!model contribution|(}
...
Note that if you need an introduction or refresher on logistic regression, you'll find it in @sec-glm-logistic.



### Marginal effects at the mean {#sec-marginal-effects-mean}

\index{marginal effects!marginal effects at the mean|(}
...
The second plot shows what is called a **partial dependence plot**\index{partial dependence plot (PDP)}, which shows the *average predicted probability* of a good review as word count changes. In both cases we make predictions with imputed values. The left plot imputes the other features to be their mean or mode, while the right plot leaves the other features at their actual values, and then, using a range of values for word count, gets a prediction as if every observation had that value for word count. We then plot the average of the predictions for each value in the range.
...
When word count is zero, i.e., its mean, and everything else is at its mean/mode, we'd predict a chance of a good review at about `r label_percent()(pred_0)`.  The average prediction we'd get if we predicted every observation as if it were the mean word count is more like `r label_percent()(avg_pred_0)`, which is notably less. Which is correct? Both, or neither!  They are telling us different things, either of which may be useful, or not.  If it's doubtful that the feature values used in the calculation are realistic, e.g., everything at its mean at the same time, or an average word count when length of a movie is at its minimum, then they may both be misleading. You have to know your features and your target to know how best to use the information.
...
\index{marginal effects!marginal effects at the mean|)}
\index{marginal effects!average marginal effects|(}
\index{average slope|see{marginal effects, average marginal effects}}
...
\index{marginal effects!marginal means|(}
...
\index{counterfactual predictions|(}
...
Let's try a new dataset to help drive the point home.  We'll use some data at the global stage: the world happiness dataset (@sec-dd-world-happiness-report). For our model we'll predict the happiness score for a country, considering freedom to make life choices, GDP and other things associated with it.  We'll then switch the freedom to make life choices and GDP values for the US and Russia, and see how the predictions change!
...
\index{SHAP values|(}
\index{Shapley values|see{SHAP values}}
...
ggsave(
    filename = "img/knowing-shap-waterfall.svg",
    plot = wf,
    device = "svg",
    width = 8,
    height = 6
...
![SHAP waterfall plot.](img/knowing-shap-waterfall.svg){#fig-shap-viz-r}
:::

:::{.content-visible when-format='pdf'}
![SHAP waterfall plot.](img/knowing-shap-waterfall.svg){#fig-shap-viz-r width=75%}
:::
...
![Two plots showing the importance of understanding feature interactions and nonlinear relationships.](img/knowing-feature-importance.svg){#fig-feature-importance width=100%}
...
\index{SHAP values!for feature importance|(}
...
p_dat_shap |>
    ...
    geom_col(
        aes(
            ...
            alpha = I(ifelse(feature == "word_count", 0, 1)),
            linewidth = I(ifelse(feature == "word_count", 0, .1))
        ),
...
p_no_decompose = pdat_no_decompose |>
    ...
    geom_col(
        aes(
            color = feature == "word_count",
            fill  = feature == "word_count",
            alpha = I(ifelse(feature == "word_count", .25, 1)),
        ),
        width = I(ifelse(feature == "word_count", .25, .1)), # says it ignores this, but doesn't
        # alpha = .25,
        show.legend = FALSE
    ) +
    scale_color_manual(
...
ggsave("img/knowing-feature-importance-2.svg", width = 8, height = 8)
```

![Feature importance by various methods.](img/knowing-feature-importance-2.svg){#fig-feature-importance-2 width=100%}
...
We also recommend checking out Christoph Molnar's book, [Interpretable Machine Learning](https://christophm.github.io/interpretable-ml-book/). It is a great resource for learning more about model explainers and how to use them, and it provides a nice package that has a lot of the functionality we've shown here.

The [marginal effects zoo](https://marginaleffects.com/), written by the [marginaleffects]{.pack} package author, is your go-to for getting started with marginal effects, but we also recommend the [excellent blog post by Andrew Heiss](https://www.andrewheiss.com/blog/2022/05/20/marginalia/) as a very nifty overview and demonstration.
