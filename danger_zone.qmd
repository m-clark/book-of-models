...
Other commonly used metrics, like AIC, might be better in theory, but they approximate the model selection one would get through cross-validation, so why not just do the cross-validation? Furthermore, as long as you are using those metrics only on the training data, you really aren't getting a good idea of how the model will generalize (@sec-ml-sec-ml-generalization).
