...
Alternatively, $R^2$  can be calculated as the squared correlation between the target and predicted values, which may be more conceptually intuitive. In that sense it can almost always be useful as a *descriptive* measure, just like we use means and standard deviations in exploratory data analysis. However, it is not so great at telling us about predictive quality. Why? Take the predictions from our rating model, and add 10 to them, or make them all negative. In both cases, if you calculate it as the squared correlation of your predictions and target, even though your predictions would be ridiculous, but your $R^2$ will be the same. If you use the formula, you could even get negative values! Another problem is that for training data, $R^2$ will always increase as you add more features to your model, whether they are useful or pure noise! This is why we use other metrics to assess predictive quality[^r2test].
...
- **Predictive Check**: This is a basic comparison of predicted vs. observed target values, and we may be getting ahead of ourselves to understand this completely yet, but it's worth knowing about. In the simplest case you can just examine your predictions vs. the observed values, and that's plenty for a quick assessment. 
...
p_pp_simple = tibble(
    ...
    labs(
        x = "Rating",
        y = "Density",
        title = "Posterior Predictive Check for a regression model"
    )
...
A proper posterior predictive check is a bit more involved, but there are packages that make it straightforward[^ppcpack]. The basic idea is that we simulate the target based on the model parameter estimates and their uncertainty, and with that we can simulate random draws of predicted values. We then compare that distribution to the observed target distribution. If the two distributions are similar, then the model is doing a good job of capturing the target distribution. This plot is ubiquitous in Bayesian modeling, but can be used for any model that has uncertainty estimates or is otherwise able to generate random draws of the target distribution. For our regression model, our predictions match the target distribution well.

[^ppcpack]: The [performance]{.pack} package in R has a `check_predictions` function that can do this for you, but base R has a `simulate` function that can be used to generate random draws of the predictive distribution for `lm/glm`  models.
...
Now that you've gotten a grasp of how to understand the model from a general perspective, you can now focus on understanding more how about how features relate to the target and make predictions. Head to @sec-knowing-feature to find out more!
