...
Now how might we do this for modeling purposes? Very easily with modern packages.  In the following we de
...
learner$param_set$values$alpha = 1 # set the penalty parameter to some value
...
# result$score(msr('classif.acc')) # show the accuracy score for each fold

# print the average accuracy score
result$aggregate(msr('classif.acc'))
```

:::


In each case above, we end up with five separate accuracy values, one for each fold. Our final assessment of the model's accuracy is the average of these five values.  This is a better estimate of the model's accuracy than if we had just used one test set.

One problem with the previous approach is that we set the penalty parameter to a fixed value. We can do better by searching over a range of values instead. This is known as **hyperparameter tuning**, or simply **tuning**.  We can do this with cross-validation as well where we will us k-fold cross-validation to assess the error for each value of the penalty parameter.  We then select the value of the penalty parameter that gives the lowest error. This is a form of as **model selection**.
...
grid_search = GridSearchCV(LogisticRegressionCV(), param_grid=param_grid, cv=5, scoring='accuracy')
...
best_param = grid_search.best_params_['Cs']
...
param_set = ParamSet$new(list(
  ParamDbl$new("alpha", lower = 1e-3, upper = 1)
))
...
<!-- 
If we are using the same data to both select the model and assess its performance.  This can be an issue because we are using the same data to both select the model and assess its performance.  This is a form of a more general phenomenon of **data leakage**.  One solution is to split the data into three sets: training, validation, and test.  We use the training set to fit the model, the validation set to select the model, and the test set to assess the model's performance.  This is known as the **train-validate-test** split.  The validation set is used to select the model, and the test set is used to assess the model's performance.  This is a form of **model selection**. -->
...
Here is an example of a pipeline in Python.  We use the `make_pipeline` function from the `sklearn` package.  This function takes a series of steps as arguments, and then performs them in sequence.  We can then use the pipeline to fit the model, assess its performance, and save it for later use[^sklearnmods]. With R, mlr3 works in a very similar fashion, which is why we use it here.  We create a pipeline with the `po` (pipe operator) function, which takes a series of steps as arguments, and then performs them in sequence. 

[^sklearnmods]: It's a bit annoying that we literally have to import a different model for each step in the pipeline, but that's the way it is.
...
Python is the king of ML. Many other languages can perform ML and maybe even well, but Python is the most popular and has the most packages. 
...
R is actually great at ML. The tools are not as fast or memory efficient, but they are typically more user friendly the tools work the same as what you would use elsewhere in the R world. They also usually have good to even excellent documentation. Some Python packages such as xgboost and lightgbm have concurrent development in R, but even then the R development typically lags with feature implementation. When it comes to ML with deep learning models, R packages merely wrap the underlying Python packages. In general though, for everything before and after ML, from feature engineering to visualization to reporting, R is the way to go. 
