...
We've found that most discussions of **bias** and **variance** and their 'tradeoff' can be difficult to digest. To begin, they start with these two very loaded words that are used in a variety of contexts, some of which are directly related to machine learning, sometimes not, but even when they are, they do not necessarily mean the same thing[^csbias], and so it's confusing from the get go. The main thing to get across here at the start is that we're specifically focused on prediction. In that context, bias and variance are two sources of error in our prediction, among others. We focus on bias and variance and not the others because we actually have control over those, while the other sources of prediction error are due to random variation, systemtic bias (known as measurement error in psychometrics), and model misspecification. The other point to keep in mind regarding to the general concept is that we are talking about the average of these errors, not the error for a particular model. But since we don't usually average our results over many hypothetically produced datasets, this is also a source of confusion. We will try to clarify these points here by keeping things simpler.
...
- They just figured out recently that keeping feature names was a good ideas
...
- easy to use objects
- easy to extract key estimated parameters
