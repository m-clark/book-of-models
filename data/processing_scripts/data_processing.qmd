...
# df_happiness |>
#     filter(str_detect(country_name, 'Somali'))
df_happiness = df_happiness |> 
    ...
    mutate(
        cntry_code = countrycode::countrycode(country, 'country.name', 'iso3c'),
        cntry_code = ifelse(country == 'Kosovo', 'XKX', cntry_code)
    ) 

df_happiness |>
    select(matches('c(.)*ntry')) |> 
    distinct() |> 
    filter(str_detect(country, 'Kosovo|Somali'))
...
df_happiness = df_happiness |> 
    ...
    left_join(
        ...
        rename_with(\(x) str_c(x, "_sc"), .cols = -c(year, country, cntry_code))
    )
...
```{r}
# just add scaled values for key features
df_happiness_2018 = read_csv('data/world_happiness_2018_raw.csv') 

df_happiness_2018_sc = df_happiness_2018 |>
    select(
        country,
        happiness_score,
...
df_happiness_2018 |> 
    left_join(df_happiness_2018_sc |> select(-happiness), by = 'country') |> 
