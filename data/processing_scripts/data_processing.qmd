...
positive_reviews = readr::read_tsv("data/positive.tsv")[[1]]
neutral_reviews = readr::read_tsv("data/neutral.tsv")[[1]]
negative_reviews = readr::read_tsv("data/negative.tsv")[[1]]
...
generate_review = function(rating) {
    reviews = case_when(
        rating > 3.5 ~ sample(positive_reviews, 1),
        rating < 2.75 ~ sample(negative_reviews, 1),
        TRUE ~ sample(neutral_reviews, 1)
    )
...
generate_data = function(n, seed = 42) {
    ...
    df_movies = df_movies |> 
        mutate(
            ...
            length_minutes = rpois(n_movies, 100), 
    ...
    rating = (
        .1    * scale(df$release_year)
        + .1  * scale(df$review_year)
        + .3  * scale(df$length_minutes) - .05 * scale(df$length_minutes)^2 # will have to tinker with this
        + .3  * scale(df$genre == 'Drama')
        - .3  * scale(df$genre == 'Comedy')
        + .2  * scale(df$season == 'Summer')
        + .3  * scale(df$total_reviews)
        - .1  * scale(df$age) 
        + .1  * scale(df$children_in_home > 0)
        + .25 * scale(df$work_status == 'Student')
        - .1  * scale(df$education == 'Post-Graduate')
        # simple interactions and hidden effects
        # split the data to more high vs. low (but not too much )
        + .25 * scale(df$reviewer_type == 'Viewer')
        - .25 * scale(df$reviewer_type == 'Critic')
        # genre interactions
        + .25 * scale(df$genre == 'Kids' & (df$children_in_home > 0))
        + .1  * scale(df$genre %in% c('Romance', 'Horror') & (df$age < 30))
        + .1  * scale(df$genre == 'Comedy' & df$work_status == 'Student')
        + rnorm(nrow(df), 0, .1)
    ) 

    # df$rating = rating[,1]
    df$rating = round(scales::rescale(rating[, 1], to = c(1, 5)), 1)
    
    df = df |> 
        mutate(
            review_text = generate_review(rating),
            word_count = str_count(review_text, '\\w+'),
            character_count = nchar(review_text)
        )
    df
}
```
