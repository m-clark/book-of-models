...
generate_data = function(n, seed = 42) {
    ...
    rating = (
        ...
        + .5 * multiplier * scale(df$reviewer_type == 'Viewer')
        - .5 * multiplier * scale(df$reviewer_type == 'Critic')
        # genre interactions
        + .25 * multiplier * scale(df$genre == 'Kids' & (df$children_in_home > 0))
        + .1  * multiplier * scale(df$genre %in% c('Romance', 'Horror') & (df$age < 30))
        + .1  * multiplier * scale(df$genre == 'Comedy' & df$work_status == 'Student')
        + rnorm(nrow(df), 0, .1)
    ) 
