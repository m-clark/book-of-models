...
For our first linear model, we'll keep things simple.  Let's predict the rating from the length of the review.  We'll use the `lm()` function in R and the `ols()` function in Python to fit the model.  The `lm()` function takes a formula as its first argument, which is a way of expressing the relationship between the features and target.  The formula is expressed as `y ~ x`, where `y` is the target and `x` is the feature. We also need to specify what the data object is.  For Python, the `ols()` function in Python works largely the same way.
...
Let's look again at our coefficient table. The following is a programming language agnostic presentation, but you can verify that it follows the previous output.
...
rss, rsq, pp_check, compare model

We can also look at the **residual variance** and **R-squared**.  The residual variance is a measure of how well the model fits the data.  It's the sum of the squared differences between the actual target values and the predicted target values.  The R-squared is a measure of how much of the variance in the target is explained by the model.  It's the proportion of the variance in the target that is explained by the model.  In this case, the residual variance is `r round(sum(resid(model_reviews)^2), 2)`, and the R-squared is `r round(summary(model_reviews)$r.squared, 2)`.  We can see that the model explains `r round(summary(model_reviews)$r.squared * 100, 2)`% of the variance in the target, which is pretty good for a single feature model.  We'll talk more about these later, but for now, we can see that the model fits the data pretty well.
...
UPDATE THIS TABLE, NOT CLEAR ENOUGH

```{r}
#| echo: false
#| eval: false
# gt doesn't seem to have an actual working answer to latex within cells, though kabel_extra and modelsummary appear to handle it as expected.
tibble(
...
For example, to start with the normal linear model situation, a one-unit change in $x$, i.e. $\Delta x =1$, leads to $\beta$ unit change in $y$. If we log the target variable $y$, the interpretation of the coefficient for $x$ is that a one-unit change in $x$ leads to an (approximately) 100$\cdot$$\beta$% change in $y$. The 100 changes the result from a proportion to percentage change. More concretely, if $\beta$ was .5, a unit change in $x$ leads to (roughly) a 50% change in $y$. If both were logged, a percentage change in $x$ leads to a $\beta$ percentage change in y.  These percentage change interpretations are called [elasticities](https://en.wikipedia.org/wiki/Elasticity_(economics)) in econometrics and areas trained similarly.
