...
MOVE THIS TO BEFORE INTERPRETATION, TOO MUCH ASSUMES IT


Once we have a working model, we can use it to make predictions. It may not seem like much at first, but a model is of no use if it can't be used to make predictions about what we can expect in the world around us. We can obtain our predictions by plugging in values for the features and using the corresponding weights. Let's go back to our results, starting with a simpler depiction.
...
We can add more features to our model very simply. Using the standard functions we just add them to the formula (both R and statsmodels)
