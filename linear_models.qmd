...
<!-- TODO: update relative to phd/isa's concerns -->
...
<!-- there is no explanation coefficients, visuals with out prediction! -->

In the following example we show the results of a model with 10k observations and a single feature predicting a binary target. The coefficient is statistically significant, and we can see a slight positive relationship. Under certain settings, such as the exploration of causal effects where we have controlled for various other factors to obtain this result, we might be satisfied with interpreting this positive relationship. 
```{r}
#| echo: false
...
get_power = function(reps = 1000) {
    pvals = replicate(reps, {
        y = rbinom(n, 1, plogis(.05*x))
        model = glm(y ~ x, family = 'binomial')
        summary(model)$coefficients['x', 4]
    })
    mean(pvals < .05)
}
power = scales::label_percent()(get_power())
# summary(model)
#confint(model)['x',]
y_pred0 = predict(model, data.frame(x=0), type='response')
y_pred1 = predict(model, data.frame(x=1), type='response')

prob_diff = scales::label_percent()(y_pred1 - y_pred0)

pdat = plot(ggeffects::ggpredict(model, terms = 'x'))$data
ggeffects::ggpredict(model, terms = 'x') |> 
    ...
    geom_point(
        data = tibble(x = -4, predicted = c(y_pred0, y_pred1)),
        color = 'red'
    ) 
...
But if we are interested in prediction, we would be disappointed with the model's performance. It predicts the target at almost the same rate as guessing even on the data it's fit on, and does even worse than guessing on new data. Even the effect as shown is quite small by typical standards, as it would take a standard deviation change in the feature to get a ~`r prob_diff` change in the probability of the target. If we are concerned solely with explanation, we now would want to ask ourselves if the effect is large enough to be of interest, and if the result is useful to make decisions regarding[^seedsofchange]. It may very well be, maybe the target concerns the percentage of survival, or the data circumstances demand such interpretation, because it is extremely costly to obtain more. For more exploratory efforts however, this sort of result would likely not be enough to come to any explanatory conclusion even if explanation is the only goal. 

[^seedsofchange]: This is a contrived example, but it's not far from what you might see in practice. The relationship is weak, and though statistically significant, the model can't predict the target well at all. The statistical power is actually decent in this case, roughly `r power`, but this is only because the sample size is so large and we have no other features to contend with. This is a common issue in many academic fields, and it's why we always need to be careful about how we interpret our models. In practice, we would need to consider other factors, such as the cost of a false positive or false negative, the cost of obtaining the feature, and the cost of the data and running the model itself, to determine if the model is worth using.

<!-- Change to an election example? <50 data points, but want to explain factors that lead to a win.  -->


Consider the world happiness data we'll be using later. We want to explain the association of country level characteristics and the population's happiness. We likely aren't going to be interested in predicting next year's happiness score, but rather what attributes are correlated with a happy populace. In this case, explanation is the focus, and we may not even need a model at all to come to our conclusions.

<!-- However, the distinction between prediction and explanation is a bit problematic, not the least of which is that we often want to do both. It is also the case that we cannot get explanation without prediction, which is what provides the coefficient estimates and a means to visualize the relationships between the features and the target. In addition, most of the time where the focus is on prediction, if we can't explain the results we get, we will typically feel fairly dissatisfied, and may question how well the model is actually doing. -->
