...
#| label: compare-error
tibble(
    zero_sq_sum = sum(error_zeros^2),
    mean_sq_sum = sum(error_mean^2)
) |>
    gt()
```
...
The resulting value - the sum or mean of the squared errors, is sometimes referred to as our **objective** or **loss** function.  We can use this value to find the best parameters for a specific model, as well as compare different models with different parameters, such as a model with different features.  We can also use this value to compare different types of models, such as a linear model and a decision tree model.  
...
ols <- function(x, y, b0, b1, sum = TRUE) {
    ...
    # Calculate the loss as sum or average
    if (sum) {
        loss <- sum(error^2, na.rm = TRUE)
    } else {
        loss <- mean(error^2, na.rm = TRUE)
    }
...
# Calculate the loss for each guess
guesses <- guesses %>%
    mutate(loss = map2_dbl(
...
guesses |>
    ggplot(aes(x = b0, y = b1, fill = loss)) +
    geom_tile() +
    geom_point(
        data = guesses %>% filter(loss == min(loss)),
        size = 6,
        color = "white"
    ) +
    geom_text(
        data = guesses %>% filter(loss == min(loss)),
        aes(label = paste0("Minimum at (", round(b0, 2), ", ", round(b1, 2), ")")),
        size = 6,
        hjust = -0.1,
        color = "white"
    ) +
    ...
    labs(
        x = "b0",
        y = "b1",
        title = "Loss for different guesses of b0 and b1"
    )
```

Returning to our results from the built-in functions, we had estimates of ...

```{r}
#| echo: false
paste(round(coef(model_reviews), 3))
```


 We'll use the `lm()` function in R and the `LinearRegression()` function in Python.  We'll also use the `summary()` function in R and the `statsmodels.summary()` function in Python to get a summary of the results.
