...
# failed attempt to fold output; see https://github.com/quarto-dev/quarto-cli/issues/341;
# despite implication on GH issue, there is no solution for not showing code but toggling output
...
gt(broom::tidy(model_reviews, conf.int = TRUE) |> rename(feature = term))
int = round(coef(model_reviews)[1], 2)
...
In the past, and even the present, statistical is focused on a great deal, even to the point that a papers are written about models that have no predictive power at all. In those settings, statistical significance is often used as a proxy for importance, which it should never be. If we are very interested in the coefficient, it is better to focus on the range of possible values, which is provided by the **confidence interval**.  While a confidence interval is also a loaded description of a feature's relationship to the target, we can use it in a very practical way as a range of possible values for that weight. Here we see that the coefficient could be as low as `r rl_ci[1]` or as high as `r rl_ci[2]`, which might interpret as a relatively narrow range, giving us some 'confidence' that we should expect a non-zero value when assessing this particular relationship. Like statistical significance though, it is affected by many things, and without context, is limited in the information it can provide.
...
In our linear model, one of, and so far the only, key **parameter** is the coefficient for each feature. But how do we know what the coefficients are? When we run a linear model using some function, they appear magically, but it's worth knowing a little bit about how they come to be.

### Guessing

As a starting point we can guess, but how would we know what to guess? How would we know which guesses are better than others?  Let's try a few guesses and see how they do.  We'll start with a guess of 0 for all the coefficients.  We can plug this into the model and see what we get: 

$$\hat{y} = 0 + 0x_1 + 0x_2 + ... + 0x_n$$

That's just going to equal zero, which we know would not be a good guess.  A better guess would just be the mean of the target.  We can plug that into the model and see what we get:

$$\hat{y} = \bar{y} + 0x_1 + 0x_2 + ... + 0x_n$$

In this case, our guess for the coefficients are still zero, but our offset (or intercept) is the mean of the target.  This is a better guess, but it's still not great.  How do we know it's better?  We can compare the predictions from each guess to the actual values of the target.  We can do this by calculating the **prediction error** or **residuals**.  The residual is the difference between the actual value of the target and the predicted value of the target.  We can express this as:

$$e = y - \hat{y}$$

Where $e_i$ is the residual for observation, which we can think of as e for error.
