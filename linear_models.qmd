...
model_rmse = round(performance::rmse(model_reviews), 2)
...
model_reviews_metrics = performance::performance(model_reviews)
rsq = round(model_reviews_metrics$R2, 2)
rsq_perc = 100 * rsq
mse = round(model_reviews_metrics$RMSE^2, 2)
rmse = round(model_reviews_metrics$RMSE, 2)
mae = round(performance::mae(model_reviews), 2)
aic = round(model_reviews_metrics$AIC, 2)
```
...
mre_metrics = performance::performance(model_reviews_extra)
mre_mae = performance::mae(model_reviews_extra)
```
...
avg_pred = mean(predict(model_reviews))
...
explainer = DALEX::explain(model_reviews, verbose = FALSE)
...
<!---
 Need to review this. even though the code is taken from a previous demo that shows equivalence (and still does for that data situation), for whatever reason, it's slightly different here due to a different baseline value, which *should* just be the mean rating. How that gets screwed up with the simplest model is beyond me, but not the first/last issue with shap package. The closest I could get was an average prediction when X != 12, and it still wasn't exact, and which doesn't make sense as a baseline.
--->
...
# first we need to get the average prediction
avg_pred = model_reviews.predict(df_reviews).mean()

# then we need to get the prediction for the feature value of interest
pred_observation = model_reviews.predict(df_reviews.assign(word_count = 12)).mean()

# then we can calculate the shap value
shap_value = pred_observation - avg_pred

# we can also use the shap package for this; it does not work with statsmodels though,
...
x_wc = df_reviews['word_count'].values.reshape(-1, 1)
x12 = np.array([12.0])

model_reviews = LinearRegression().fit(x_wc, df_reviews['rating'])

explainer = shap.explainers.Linear(model_reviews, x_wc, algorithm='exact')

explainer.shap_values(x12)
```
...
tibble(
    ours = shap_value,
    dalex = as_tibble(shap_value_package[1, ])[["contribution"]]
    # dalex = data.frame(shap_value_package['word_count', ])[["contribution"]]
) |>
    gt(decimals = 3)
```
...
```{r}
# round(coef(model_reviews_extra), 3)
# pp
```
...
explainer <- DALEX::explain(
    model_reviews_extra,
    data = insight::get_data(model_reviews_extra) |> sample_n(1000),
    y = df_reviews$rating,
    verbose = FALSE
)

observation_of_interest <- tibble(
    word_count = 12,
    age = 30,
    children_in_home = 1,
    length_minutes = 100,
...
# pp |> data.table::as.data.table() |> filter(str_detect(variable, 'word_count'))

tibble(pp["word_count", ][1:2])
pp
pp |> plot()
# plot(DALEX::model_performance(explainer, model_reviews_extra))
```

```{r}
# map2_df(
#     .x = observation_of_interest,
#     .y = colnames(observation_of_interest),
#     \(x, y)
#     mean(
#         predict(model_reviews_extra, newdata = insight::get_data(model_reviews_extra) |> mutate(!!y := x)) - mean(predict(model_reviews_extra))
#     )
# ) |>
#     dplyr::select(sort(colnames(observation_of_interest)))
...
bd = sv_force(plot_dat_shap, fill_colors = fc, colour = "white", contrast = FALSE) +
    labs(
        x = "Breakdown",
        title = "SHAP Force"
    )
...
We have a lot of visual tools at our disposal to help us understand our model.  We've already seen a few, but let's look at some more to help understand our feature - target relationship.
...
As before we can move beyond feature level interpretation
