...
![Linear regression of rating vs. word count](img/lm-word_count_linear_model.svg){width=75% #fig-word-count-linear-model}
...
![Linear regression as a graphical model.](img/graphical_lm.svg){width=75% #fig-graph-lm}
...
\index{interval!prediction} \index{interval!confidence}
...
In the end we'll need to balance our efforts to suit the task at hand. Prediction and explanation are both fundamental to the modeling endeavor. We return to this topic again in the chapter on causal models (@sec-causal-prediction-explanation). \index{prediction!vs. explanation|)}
