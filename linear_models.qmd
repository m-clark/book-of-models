...
model_reviews_multi = lm(
    rating ~ review_length + number_of_reviews + year_of_release,
    data = df_reviews
)

our_estimate = ols(X = X, y = df_reviews$rating, par = coef(model_reviews_multi))
lm_estimate = mean(resid(model_reviews_multi)^2)
```
...
model_reviews_multi = smf.ols(
    'rating ~ review_length + number_of_reviews + year_of_release', 
    data = df_reviews
).fit()

our_estimate = ols(X = X, y = df_reviews['rating'].to_numpy(), par = model_reviews_multi.params.to_numpy())
