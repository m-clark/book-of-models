...
There are a lot more fun things we can do while still employing a linear model. We can add **interactions** between features, which is a way to account for the fact that the effect of one feature may depend on the value of another. We can also account for non-linear relationships. In addition, we can take steps to enhance our prediction with an otherwise identical approach to the models shown in this chapter. We'll talk more about these throughout the rest of Parts I and II.
