...
where $y$ is the target, $X$ is a @d matrix of features[^interceptones], where the rows are observations/instances and columns features, and $\beta$ is a vector of coefficients the same length as the number of columns in $X$. Matrix multiplication provides us an efficient way to get our expected value/prediction. 
