...
model_train = smf.ols(
    formula = "rating ~ review_year_0 + release_year_0 + age_sc + length_minutes_sc + total_reviews_sc + word_count_sc", 
    data = training_data
).fit()
```
...
In the end, it won't hurt to look at a few of these metrics to get a better idea of how well your model is performing. You will always be using these metrics to compare different models, so use a few of them to get a better sense of how well your models are performing relative to one another.
...
logistic_model_train = smf.glm(
    formula = "rating_good ~ review_year_0 + release_year_0 + age_sc + length_minutes_sc + total_reviews_sc + word_count_sc", 
    data = training_data, 
...
We are going to take those probability values and make a decision to convert everything above .49 to the positive class (a "good" review). It is a bold assumption!
...
We can get almost all of that with the `confusionMatrix` function from the `caret` package:
...
train_var_effect = train_explain.model_profile(
    features
)
...
break_down_plot = train_explain.predict_parts(
    new_observation = testing_data.iloc[1, :], 
    type = "break_down"
)
