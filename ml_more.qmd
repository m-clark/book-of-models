...
The prospect of combining models for computer vision and natural language processing can produce tools that mimic many aspects of what we call intelligence. Current approaches can pass law and medical exams, create better explanations of images and text, and produce conversation on par with humans.  AI even helped to create this book!

From a modeling aspect, many put AI as the general approach to modeling, and ML as a subset of AI, but this is a bit off the mark in our opinion. For example, model-wise, AI only employs deep learning models, while the ML approach to training models can be used for any underlying model. 

and AI uses the ML approach of training on data with the goal to generalize performance.  Many of the non-AI settings we use modeling for may well be things we can eventually rely on AI to do, but the computational limits, whether the amount of data that would be required for AI models to be better than others, or the ability of AI to be able to deal with situations in which there is *only* small bits of data, are still hinderances in application. However, it's likely these will eventually be overcome.
