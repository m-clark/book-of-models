...
ggsave("img/lm-extend-linear_effect.png")
```
:::
:::{.content-visible when-format="pdf"}
![A standard linear effect](img/lm-extend-linear_line.png){#fig-regular-linear-line}
:::
...
:::
:::{.content-visible when-format="pdf"}
![A curvilinear effect](img/lm-extend-gam_line.png){#fig-gam-model-line}
:::
...
plot(
  modelbased::visualisation_recipe(
    ...
    point  = list(color = okabeito_colors()[1])
    line   = list(color = okabeito_colors()[2])
  ),
  alpha = 0.5
) +
  labs(
...
ggsave("img/lm-extend-gam_plot.png")
```
:::

:::{.content-visible when-format="pdf"}
![Visualizing a GAM](img/lm-extend-gam_plot.png){#fig-gam-plot}
:::
...
> Oh, you think the median is your ally. But you merely adopted the median. I was born in it, molded by it. I didn't see anything interesting until I was already a statistician, and by then it was nothing to me but misleading.
> -- Bane (probably)
