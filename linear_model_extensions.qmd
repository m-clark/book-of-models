...
<!-- TODO: This can be a chapter with a general focus on nonlinearities: interactions in general and mixed model, gam (effects vary with self or other), quantile (effects vary with target). Also remind GLM as introducing nonlinearity. -->
...
ggsave("img/lm-extend-interaction_plot.png")
```
:::

::: {.content-visible when-format="pdf"}
![Interaction plot](img/lm-extend-interaction_plot.png){#fig-model-interaction-plot}
:::
...
ggsave("img/lm-extend-length_release_rating.png")
```
:::
:::{.content-visible when-format="pdf"}
![Linear relationship between length of movie and rating](img/lm-extend-length_release_rating.png){#fig-length-release-rating}
:::
...
p_slopes = ggplot(mods, aes(length_minutes, .fitted, color = genre)) +
  geom_line(aes(linetype = genre), lwd = 1) +
...
ggsave("img/lm-extend-length_genre_rating.png")
...
:::
:::{.content-visible when-format="pdf"}
![Genre Effects on Length and Rating](img/lm-extend-length_genre_rating.png){#fig-length-genre-rating}
:::
...
ggsave("img/lm-extend-length_release_rating_year.png")

```
:::
:::{.content-visible when-format="pdf"}
![Release Year Effects on Length and Rating](img/lm-extend-length_release_rating_year.png){#fig-length-release-rating-year}
:::
...
To use mixed models, at a minimum we have to specify a group effect in some way, but that's the primary difference from our previous approaches used for linear or generalized linear models. We can do this in a few different ways, but we'll start with the simplest way. We do it for having just a random intercept for genre, and then we'll add a random slope for length by genre. In both cases we use the standardized version of length, as we have done elsewhere.
...
@tbl-mixed-model shows some typical output from a mixed model, focusing on the random slope model. The fixed effect part (Fixed) is your basic GLM result and interpreted the same way. Nothing new there, and we can see a general positive relationship between length and rating, though maybe not a strong one. But the random effects (Random) are where the action is! We can see the standard deviation of the random effects, i.e., the intercepts and slopes. We can also see the standard deviation of the residuals, which  is conceptually the same as what you saw with standard linear regression. We can also see the correlation between the random intercepts and random slopes. Depending on your tool, the default may be in terms of variances and covariances rather than standard deviations and correlations, but otherwise the same.
...
ggsave("img/lm-extend-random_effects.png")
```
:::
:::{.content-visible when-format="pdf"}
![Random effects for mixed model](img/lm-extend-random_effects.png){#fig-random-effects}
:::


How do we interpret these *deviations*? For starters, they are deviations from the fixed effect for the intercept and slope. So here that value is zero, and anything negative is below that value, and anything positive is above that value. If we want the specific effect for a genre, we just add the random effect to the fixed effect. So, for example, if we wanted to know the effect of the kids genre, we would add the random effect for the kids genre to the fixed effect for the intercept. This is exactly what we did in the previous section with the interaction model.

For the intercept plot, we see that Kids, Sci-Fi, and Action/Adventure have a lower default value for rating, while Drama and Comedy start off relatively higher. These values reflect what's happening when the length is zero, which, since it's standardized, means that we're talking about what's happening for an average length movie.

The length effect for comedy suggests that it has a smaller slope, and relative to the global coefficient, this suggests that comedies can wear out their welcome faster than other genres. Longer romance movies, on the other hand, have an even larger positive coefficient, and so seem to do better seem to do better than short ones- maybe all the short ones are mostly awful Rom-Coms!

:::{.callout type='tip' title='Always scale features for mixed models' collapse="true"}
...
For a model with just one feature, we certainly had a lot to talk about! And this is just a glimpse of what mixed models have to offer, and the approach can be even richer than what we've just seen.  But you might be asking- Why don't I just put genre into the model like other categorical features? In the case of genre, that's okay. Now consider thousands of United States county voting percentages for elections over time- would you just put 3000+ county indicator variables into the model as is? You can try, but you'll likely run into estimation problems for typical GLM settings. 

In addition, mixed models help explore the correlation of group effects, as well as mixed models estimate the correlation of the observations within groups. Default mixed models assume this correlation is constant, but this can be modified to allow for different correlation structures. For example, in a longitudinal study you might want to assume that the correlation between observations within a group decreases as the time between observations increases. This is a very common approach for longitudinal data, where the correlation between observations decreases as the time between observations increases.
...
Such models belong to a broad group of *generalized additive models* (**GAM**s). When we used an interactions, we explored how the feature-target relationship varies with another feature. When we fit mixed models and interactions earlier, we focused on our feature and its relationship to the target at different values of other features. When we use a GAM, we are going to focus on our feature, and see how the relationship changes at different values for it. How are we going to do this, you might ask? Conceptually, we will have a model that looks like this:
...
Now that you have some background, let's give this a shot! In most respects, we can use the same sort of approach as we did with our other linear model examples. For our example here, we'll use model what was depicted in figure @fig-gam-model-line, which looks at the relationship between the `healthy_life_expectancy_at_birth` and `happiness_score` variables from the world happiness data.
...
Far more important than any of these is the visual interpretation, and we can get plots from GAMs easily enough (results not shown).
