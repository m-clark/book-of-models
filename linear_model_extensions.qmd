...
ggplot(df_reviews, aes(total_reviews, rating)) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal()
```
...
gam_model = gam(rating ~ s(total_reviews_sc, bs = "cr"), data = df_reviews)
...
X = pd.DataFrame({'total_reviews_sc': df_reviews['total_reviews_sc']})
...
ggplot(df_reviews, aes(total_reviews, rating, color = as.factor(genre))) +
  # geom_point() +
  geom_smooth(method="lm", se = FALSE)
```
