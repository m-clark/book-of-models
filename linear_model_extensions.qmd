...
You'll notice right away that we have a few differences. Our quantile loss function includes the **tau** argument, which will let us set our quantile of interest; naturally, it can be any value between 0 and 1. The residual is multiplied by the tau value, only if the residual is greater than 0 -- positive values are weighted by **tau**. If the residual is negative, we need to add tau to -1 -- our negative residuals are weight by 1-**tau(**). Since we need a positive value for our loss values, we will multiply our negative residuals by the negative value produced from -1 plus our tau value. After that, we just sum all of those positive loss values and do our best to minimize that summed value. sum of weighted absolute residuals. sum of weighted absolute residuals
...
reviews$total_reviews_scaled <- reviews$total_reviews - min(reviews$total_reviews)
reviews$total_reviews_scaled <- reviews$total_reviews_scaled / max(reviews$total_reviews_scaled)
gam_model <- gam(rating ~ s(total_reviews_scaled, bs = "cr"), data = reviews)
