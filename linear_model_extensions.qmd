...
Performing a quantile regression at the median doesn't require too much more effort than a normal linear model. We can use the `quantreg` package in R or `statsmodels` in Python. Aside from a small function change, we also need to specify the quantile of interest -- `tau` in R and `q` in Python.
...
reviews = pd.read_csv("~/Documents/book-of-models/data/movie_reviews_processed.csv")
...
Fortunately, our interpretation of this result isn't all that different from a standard linear model. The intercept is the predicted value for the **tau** of y given x = 0. The feature coefficient suggests that the ratings increase by .00007 for every additional review. However, this result is interpreted as the amount of change in the conditional quantile of y given x.
...
Instead of a single model to capture the trend through the mean of the data, we can now examine the trends within 5 different quantiles of the data (we aren't limited to just those quantiles, though, and you can examine any of them that you might find interesting). If we had to put some words to our visualization, we could say that all of the quantiles show a positive relationship. While they all appear to have roughly the same slope, it appears that the 90th quantile has a slightly steeper slope than the other quantiles.

#### Prediction

In our standard, OLS-based regression model, we can use the regression results to make predictions. We can do the same thing with our quantile regression model, with some slight wrinkles to think about.

:::{.panel-tabset}

##### R

```{r, r_quantile_predict}
#| echo: true
#| label: r_quantile_predict
...
```{python, python_quantile_predict}
median_test.predict()
```

Let's look at those predictions compared to the actual values:
...
ggplot() +
  geom_point(data = reviews, 
             ...
             alpha = .5) +
  geom_point(data = reviews, 
             mapping = aes(total_reviews, predict(median_test)), 
             color = "red") +
  theme_minimal()
```
...
With a little bit of background and some results for comparison, let's see one way that we can create a function for quantile loss. We will also show the function for linear regression, just to have something for comparison purposes.
...
You'll notice right away that we have a few differences. Our quantile loss function includes the **tau** argument, which will let us set our quantile of interest; naturally, it can be any value between 0 and 1. The residual is multiplied by the tau value, only if the residual is greater than 0 -- essentially, positive values are weighted by **tau**. If the residual is negative, we need to add tau to -1 -- our negative residuals are weight by 1-**tau(**). Since we need a positive value for our loss values, we will multiply our negative residuals by the negative value produced from -1 plus our tau value. After that, we just sum all of those positive loss values and do our best to minimize that summed value.
...
You'll see that we returned the same values as our standard R and Python functions. 
...
That curved line is called a spline. Oddly enough, we can still use a linear model to fit this spline through the data. While this might not give us the same tidy explanation that a typical line would offer, we will certainly get better prediction. 
...
gam_model = LinearGAM(s(0, spline_order=3)).fit(X = reviews[''], y = reviews[''])

gam_model.summary()
```
:::

There is a substantial amount to unpack here! First, notice that the GAM functions both use **s** to produce a **spline** for that variable -- without the **s** function , we would have a typical linear regression. We also need to define what type of spline that we want to fit. In this case, we are using a **cubic regression spline**, which is a pretty common choice. We can also see that we have a **bs** argument in our R model, which stands for *basis*. The basis argument is used to specify the type of spline that we want to use. The **cr** stands for **cubic regression**. In Python, we have a **spline_order** argument, which is set to 3 to indicate that we want a cubic spline.

For the results of our gam model, one of the best places to look first is at `edf` in R or `edof` in Python. The `edf` column is the **effective degrees of freedom**. You can think of `edf` as a measure of "wiggle" in the relationship between the predictor and the target. The higher the `edf`, the more wiggle you have. If you have a value close to 1, then you have a linear relationship. With an `edf` of 8.672, we can be pretty confident that a nonlinear relationship gives a better idea about the relationship between `total_reviews` and `rating` than a linear relationship.
...
x <- reviews$total_reviews - min(reviews$total_reviews)
x <- x / max(x)
...
x = reviews['total_reviews'] - reviews['total_reviews'].min()
x = x / x.max()
rating = reviews['rating']
```
...
We can see that the relationship starts a bit flat, increases, and then flattens out again. This is a pretty good example of a nonlinear relationship.
...
There is definitely some wiggle to that line, but it is not as extreme as what we saw with our unpenalized cubic spline. 
...
In this visualization, we see a pretty solid positive relationship between the total number of reviews and ratings. We could probably just stop there, but we might be ignoring something substantial within our data: year of release. We might want to ask a question, "Does this relationship work the same way across the different years?"
...
A very quick examination of the visualization might suggest that the relationship between user age and rating varies significantly over the different genres. Most genres show a positive relationship, while one (`other`) shows a negative relationship, and all with various levels of strength.
...
Before we can fit our mixed model, we need to create a model matrix. We are going to use the `model.matrix()` function to create our model matrix. We are going to use the `user_age` variable as our fixed effect and `genre` as our random effect. We are going to use the `factor()` function to make sure that `genre` is treated as a categorical variable. We are also going to use the `-1` to remove the intercept from the model matrix. 
...
X <- model.matrix(~total_reviews, reviews)
...
Let's see if we can get a match from `lme4` or `statsmodels`. We will also create `null models`, since we will need some infomration from them later.
...
fit_mer = lmer(rating ~ total_reviews_scaled + (1 | genre), 
               reviews, 
               REML = FALSE)
...
fit_mer = sm.MixedLM.from_formula("rating ~ total_reviews_scaled", reviews, 
...
We also see some additional information for our random effects. The standard deviation is telling us how much the rating moves around based upon genre after getting the information from our fixed effects (i.e., the score can move around nearly .3 points from rating alone). 
...
model_data <- data.frame(rating = reviews$rating, 
                         total_reviews = reviews$total_reviews_scaled, 
                         genre = reviews$genre)
