...
[^polyspline]: If we penalized the coefficients of the polynomial regression, we would have a penalized spline. It's not used that much in GAMs these days, but there's nothing wrong with doing so. See @sec-estimation-penalty for more information on penalized regression.
