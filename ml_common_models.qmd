...
split = initial_split(X_num_df |> drop_na(), prop = .75)
...
Let's compare the results of our models head to head. We went back and tuned each model over different settings:
...
In our first approach, we just chose some parameters for our cross-validation. In our second approach, we used the tuned values to retrain on the complete data set. Though not necessary, we still show the results of the cross-validation for the already-tuned models, to give a sense of the uncertainty in error estimation.
...
for (i in list(lrn_glmnet, lrn_lgbm, lrn_mlp)) {
    preds = i$predict_newdata(df_test) 

    test_accs = append(
        test_accs, 
        mlr3measures::confusion_matrix(preds$truth, preds$response, positive = '1')$measures['acc']
    )
}
```
...
#| tbl-cap: Holdout results for tuned models.
#| echo: false

tibble(
    model = c('Elastic Net', 'LGBM', 'MLP'),
    `Test Acc.` = unlist(test_accs)
) |> 
    gt(decimals = 2)
```
