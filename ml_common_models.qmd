...
model_cv_elastic = cross_validate(
    model_elastic,
    X,
    y,
    cv = 5,
...
# pd.DataFrame(model_cv_elastic) # default output
```
...
pd.DataFrame(model_cv_elastic).to_csv('ml/data/elasticnet-py-results.csv', index=False)
```
...
model_cv_elastic = pd.read_csv('ml/data/elasticnet-py-results.csv')

print(
    'Training accuracy: ',
    np.round(model_cv_elastic['test_score'].mean(), 3),
    '\nGuessing: ',
    np.round(majority, 3),
)
```
...
model_cv_elastic = resample(
    task       = tsk_elastic,
    learner    = model_elastic,
    resampling = rsmp("cv", folds = 5)
)

# model_cv_elastic$aggregate(msr('classif.acc')) # default output
```
...
saveRDS(model_cv_elastic, 'ml/data/elasticnet-r-results.rds')
```
...
model_cv_elastic = readRDS('ml/data/elasticnet-r-results.rds')

# Evaluate
acc_inc = round(model_cv_elastic$aggregate(msr('classif.acc')) / prevalence - 1, 3)

glue("Training Accuracy: {round(model_cv_elastic$aggregate(msr('classif.acc')), 3)}\nGuessing: {round(majority, 3)}")
```
...
So here we have a model that is also performing well, though not significantly better or worse than our elastic net model. For most situations, we'd expect boosting to do better, but this shows why we want a good baseline or simpler model. We'll revisit hyperparameter tuning using this model later.  
...
# this will take a few seconds
model_cv_elastic_tune = RandomizedSearchCV(
    model_elasticnet, 
    param_grid, 
    n_iter=10,
    cv=5, 
...
model_cv_elastic_tune.fit(X_train, y_train)

# show the results

df_model_cv_elastic_tune = pd.DataFrame(model_cv_elastic_tune.cv_results_)
...
model_cv_elastic_tune.best_score_
...
We can tune all the models and compare them head to head. We first split the data into training and test sets (20% test). Then with training data, we tuned each model over different settings:
