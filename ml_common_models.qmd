...
![](img/chapter_gp_plots/gp_plot_7.svg){width=75% fig-align='center'}
:::
...
- Linear models, especially linear and logistic regression (@sec-foundation and @sec-glm)
...
\index{machine learning!general approach}
...
#| tbl-cap: Interval Estimates for Accuracy Differences
...
tibble(
    ...
    tab_footnote(
        md("Statistics regard the difference in proportions of .75 and .73.")
    ) |> 
    tab_options(
...
Another common linear model approach is **elastic net**\index{elastic net}, which we also saw in @sec-ml-core-concepts. It combines two techniques: lasso\index{lasso (L1) penalty} and ridge regression\index{ridge (L2) penalty}.  We demonstrate the lasso and ridge penalties in @sec-estim-penalty, but all you have to know is that elastic net combines the two penalties: one for lasso and one for ridge, along with a standard objective function for a numeric or categorical target. The relative proportion of the two penalties is controlled by a mixing parameter, and the optimal value for it is determined by cross-validation.  So for example, you might end up with a 75% lasso penalty and 25% ridge penalty. In the end though, it's just a slightly fancier logistic regression!
...
### Strengths and weaknesses {#sec-ml-common-penalized-strengths-weaknesses}
...
\index{nonlinear models!tree-based models}
\index{tree-based models|(}

![](img/tree_complex.svg){width=40% fig-align="center"}
...
![Simple classification tree.](img/tree.svg){#fig-tree width=30%}
...
### Strengths and weaknesses {#sec-ml-common-trees-strengths-weaknesses}
...
\index{nonlinear models!neural networks}
...
![Neural network.](img/nn_basic.svg){#fig-basic-nn width=50%}
...
\index{neural networks!tabular data}
...
[^hidden]: The term 'hidden' is used because these nodes are between the input or output. It does not imply a latent/hidden variable in the sense used in many statistical models, but there is common ground. See the connection with principal components analysis, for example (@sec-ml-more-pca-as-net).
...
:::{.content-visible when-format="html"}
![The first hidden layer.](img/nn_first_hidden_layer.svg){#fig-first-layer width=33%}
:::

:::{.content-visible when-format="pdf"}
![The first hidden layer.](img/nn_first_hidden_layer.svg){#fig-first-layer width=20%}
:::
...
\index{ReLU|see{activation functions}}
\index{sigmoid|see{activation functions}}
\index{hyperbolic tangent|see{activation functions}}
\index{linear activation|see{activation functions}}

[^relu]:  We have multiple options for our activation functions, and probably the most common one in deep learning is the **rectified linear unit** or [ReLU](https://en.wikipedia.org/wiki/Rectifier_(neural_networks)), and its more recent variants. Others used include the [sigmoid function](https://en.wikipedia.org/wiki/Sigmoid_function), which is the same as what we used in logistic regression, the hyperbolic tangent function, and the linear/identity function, which does not do any transformation at all.
...
:::{.content-visible when-format="html"}
![Activation function transforms node output before the next layer.](img/nn_activation2.svg){#fig-activate width=50%}
:::
:::{.content-visible when-format="pdf"}
![Activation function transforms node output before the next layer.](img/nn_activation2.svg){#fig-activate width=30%}
:::
...
:::{.content-visible when-format="html"}
![Complex neural network.](img/nn_deep.svg){#fig-complex-nn width=85%}
:::

:::{.content-visible when-format="pdf"}
![Complex neural network.](img/nn_deep.svg){#fig-complex-nn width=75%}
:::
...
\index{logistic regression!as a neural network|(}
...
:::{.content-visible when-format="html"}
![Logistic regression as a neural network with a single hidden layer with one node, and sigmoid activation](img/nn_logreg.svg){#fig-logreg-nn width=75%}
:::

:::{.content-visible when-format="pdf"}
![Logistic regression as a neural network with a single hidden layer with one node, and sigmoid activation](img/nn_logreg.svg){#fig-logreg-nn width=50%}
:::


This shows that we can actually think of logistic regression as a very simple neural network, with a  linear combination of the inputs as a single hidden node and a sigmoid activation function adding the nonlinear transformation. Indeed, the earliest **multilayer perceptron** models\index{multilayer perceptron (MLP)} were just composed of multiple layers of logistic regressions! 

\index{logistic regression!as a neural network|)}

:::{.callout-note title="GAMs and Neural Networks" collapse='true'} 
You can think of neural networks as nonlinear extensions of linear models\index{generalized additive models!vs. neural networks}. Regression approaches like GAMs and Gaussian process regression\index{Gaussian process} can be seen as [approximations to neural networks](https://arxiv.org/abs/1711.00165) (see also @rasmussen_gaussian_2005), bridging the gap between the simpler, and more interpretable linear model, and black box of a deep neural network. This brings us back to having a good baseline. If you know some simpler tools that can approximate more complex ones, you can often get 'good enough' results with the simpler models.
:::
...
\index{multilayer perceptron (MLP)|(}
...
[^fastaitab]: A really good tool for a standard MLP type approach with automatic categorical embeddings is [fastai]{.pack}'s tabular learner. For a more flexible, DIY type of approach, consider the recently developed [torch_frame]{.pack} package.
:::
...
### Strengths and weaknesses {#sec-ml-common-dl-nn-strengths-weaknesses}
...
## Tuned Example {#sec-ml-common-tuned-ex}
...
Here is an example of a hyperparameter search using the boosting model. We'll tune the number of trees, the learning rate, the minimum number of observations in each leaf, and the maximum depth of each tree. We'll use a **randomized search**\index{tuning!randomized search} across the parameter space to sample from the set of hyperparameters, rather than searching every possible combination as in a **grid search**\index{tuning!grid search}. This is a good approach when you have a lot of hyperparameters to tune, and/or when you have a lot of data.
...
test_acc = model_boost_cv_tune$predict(
    tsk_model_boost_cv_tune,
    row_ids = split$test
...
\index{model selection, comparison|(}
```{r}
...
# The return: code works but not with that saved object
...
![Cross-validation results for tuned models.](img/ml-benchmark-comparison.png){#fig-benchmark width=75%}
...
[^modcompare]: This table was based on Python with randomized CV search, but the R approach produced similar results. However, they can both vary quite a bit even with just a random seed change due to the small sample size.
...
\index{machine learning!tabular data|(}
When you research classical machine learning models for the kind of data we've been exploring, you'll find a variety of methods. Popular approaches from the past include *k*-nearest neighbors regression\index{k-nearest neighbors regression}, principal components regression, support vector machines (SVM)\index{support vector machines}, and more. You don't see these used in practice as much though for several reasons:
...
If you haven't had much exposure to statistical approaches, we suggest heading to any chapter before Chapter [-@sec-ml-core-concepts]. Otherwise, consider an overview of more machine learning techniques (@sec-ml-more), data-specific considerations (@sec-data), or causal modeling (@sec-causal).
...
- An overview of deep learning applications for tabular data by Michael [see @clark_this_2021; @clark_deep_2022]
