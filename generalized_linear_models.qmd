...
Logistic regression does not have an $R^2$ value in the way that a linear regression model does. Instead, there are pseudo-$R^2$ values, but they are not the same as the $R^2$ value that you are used to seeing. You can find a good breakdown of different pseudo methods [from UCLA's Statistical Consulting Group](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faq-what-are-pseudo-r-squareds/).
:::
...
:::{.content-visible when-format='pdf'}
![Poisson regression predictions for word count feature](img/glm-poisson-word-count.svg){#fig-poisson-regression}
:::
...
Similarly, we can also use our function to estimate the coefficients for the poisson model. Just like the glm we can change the family option to specify the distribution we want to use. 
...
And once again we're able to get the same results.
...
Use the fish data (@sec-dd-fish) to conduct a Poisson regression and see how well you can predict the number of fish caught based on the other variables like how many people were on the trip, how many children, whether live bait was used etc.
