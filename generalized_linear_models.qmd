...
At the end of the day, these link functions will convert the target to an unbounded continuous variable.

The take-away here is that the link function describes how the mean is generated from the predictors.

## Linear Regression

Linear regression is not new to us. Remember that a linear regression deals with real numbers between $-\infty$ to $\infty$ in the target variable. It is the most vanilla within the GLM -- a Gaussian distribution with an "identity" link function isn't the most exciting thing in the world.
...
We are going to return to our movie reviews data and we are going to use `rating_good` as our target. Before we get to modeling, see if you can find out the frequency of "good" and "bad" reviews. We will use `age` and `gender` as our predictors.
...
Now that we have our custom functions made, let's test them against R's `glm` function and Python's `statsmodels` function.
...
Now that we know that we can match results, we need to know what those results mean. The coefficients that we get from our model are in log odds. We can exponentiate them to get the odds ratio, but we can also exponentiate them and divide by 1 + that value to get the probability. Interpretting log odds is a fool's errand, but we can interpret the odds ratio and the probability.

When we exponentiate the coefficients, we are given the odds ratio.  This is the ratio of the odds of the outcome occurring for a one unit increase in the predictor. 

```{r}
exp(logistic_regression$coefficients)
```

We see that we've got an odds ratio of .86 for the word_count variable and 1.12 for the male variable. An odds ratio of 1 means that there is no change in the odds of the outcome occurring -- essentially that the predictor does not influence the target. An odds ratio of less than 1 means that the odds of the outcome occurring decrease as the predictor increases. An odds ratio of greater than 1 means that the odds of the outcome occurring increase as the predictor increases.
...
And visualizing those probabilities is absolutely the best way to see how the predictors influence the target:
