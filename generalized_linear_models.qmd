...
X = df_reviews[, c("word_count", "gender")]
...
model_logistic = smf.glm('rating_good ~ word_count + gender', data = df_reviews).fit()
...
Fortunately, the intercept is easy -- it is the odds of a "good" review when word count is 0 and gender is "female". We see that we've got an odds ratio of `r round(or_result[2], 2)` for the word count variable and `r round(or_result[3], 2)` for the male variable. An odds ratio of 1 means that there is no change in the odds of the outcome occurring - essentially that the feature does not influence the target. An odds ratio of less than 1 means that the odds of the outcome occurring decrease as the feature increases (while a bit more complicated to wrap your head around, it captures the idea of the odds of moving from a "bad" review to a "good" review decreasing). An odds ratio of greater than 1 means that the odds of the outcome occurring increase as the feature increases (again, the odds of moving from a "bad" review to a "good" review increasing).

It is more intuitive to interpret the probability.  We can do this by examining the predicted probabilities for different values of the features. 

```{r}
#| echo: false
#| label: fig-logistic-regression-prob
ggeffects::ggpredict(model_logistic, terms = c("word_count", 'gender')) |> 
  plot() 
```


In @fig-logistic-regression-prob, we can see a clear negative relationship between the number of words in a review and the probability of being considered a "good" movie. As we get over 20 words, the predicted probability of being a "good" movie is less than .2. 
...
X_mat = X.copy()
X_mat['male'] = (X_mat[['gender']] == 'male').astype(int)
X_mat = X_mat.drop(columns='gender')
