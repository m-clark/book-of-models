...
To aid our understanding beyond the math, we try visually express models in the form of graphical models or even in more complex ways with neural networks[^lda], as in the following images. 
...
When it comes to modeling, there are a few key steps that you should always keep in mind. These are not necessarily necessarily exhaustive, but we feel they're a good way to think about how to approach modeling in data science.

**Define the problem**

Start by clearly defining the problem you want to solve. It is often easy to express in very general terms, but it's often challenging to pin down the problem statement more precisely, in a way that can actually help you solve it. What are you trying to predict? What data do you have to work with? What are the constraints on your data and model? What are the consequences of the results, whatever they may be? Why do you even care about any of this? These are all questions you should be try to answer before  diving into modeling.
...
:::{.callout type='info' title="A List of Models" collapse="true"}
...
Thinking about families or groups of models can do a lot to help demystify the modeling process. You could come up with other schemas within a specific data domain or group of models, there's no solid rule here. But it can be helpful to compartmentalize the models so that you you don't get overwhelmed by what are often minor details that won't significantly impact the practical application.
...
Model performance is largely going to come from the quality of the data and how you've prepared it, from insuring its integrity to feature engineering. Some models will definitely work better than others in certain situations, but there are no guarantees, and often the practical difference in performance is minimal. But you can potentially improve performance by understanding your data better, and by understanding the limitations of your model. Having more domain knowledge can help reduce noise and irrelevant information that you might have otherwise retrained, can provide insights for feature engineering. Thorough data exploration can reveal bugs and issues to be fixed, and will help you understand the relationships between your features and your target.
