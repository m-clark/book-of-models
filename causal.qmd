...
**A/B testing** is generally used as a marketing term for a project focused on comparing two groups or scenarios, e.g., to see if a new marketing campaign results in a positive business outcome. It *implies* randomized assignment, but you'd have to understand the context to know if that is actually being implemented, and in a way that would allow for causal inference.
:::
...
![Covid Vaccinations and Deaths in the US](img/causal-covid-vax-deaths.svg){width=100% #fig-covid-vax-deaths}
...
- Regression discontinuity design[^regdisc]
- Difference-in-differences[^diffindiff2]
