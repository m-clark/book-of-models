...
In general, any modeling technique might be employed as part of a causal modeling approach.
...
We've been pretty loose in our presentation here, and glossed over a lot of the details. There are many sticklers for causal modeling who will find fault with our presentation, and that's okay. Our main goal is to give you some idea of the domain, but more so the models used and things to think about when you want to answer a causal question with your data.

Models used in statistical analysis and machine learning are not causal models, but when applied to the real world, a causal model is a statistical/ML model with more assumptions, with additional steps taken to adhere to those assumptions. A causal model requires these assumptions in order to make stronger causal statements, but neither the assumptions, data, nor model can prove that the underlying theory is causally correct. Nothing about employing a causal model inherently makes better predictions either, which, for better or worse, is what many seem to care about most[^collcorr].
...
Ask someone who is very into causal modeling how they know something is a collider or confounder variable, but tell them they can't use any statistical approach of any kind to determine this, especially not correlation.  Don't worry, it will be obvious to them that just because their causal model works by using it as such doesn't actually prove that it is.