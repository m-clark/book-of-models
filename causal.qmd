...
It can be difficult to meet these assumptions, and there is not always a clear path to a solution. As an example, say we want to assess a new curriculum's effect on student performance. We can randomly assign students, but they can interact with one another both in and outside of the classroom. Those who receive the treatment may be more likely to talk to one another, and this could affect the outcome, enhancing its effects if it is beneficial. This would violate our assumption of no interference between units, and we'd need to maybe choose an alternative design or model to account for this.

The following demonstrates a common assumption that is regularly guarded against in causal modeling - **confounding**. In this case our confounder `U`, is a variable that affects both treatment `X` and target `Y`. We'll generate some synthetic data with a confounder, and fit two models, one with the confounder and one without. We'll compare the coefficients of the feature of interest in both models.
...
These methods are not models themselves, and potentially can be used with just about any model that attempts to estimate the effect of a treatment. An nice overview of using such methods vs. standard regression/ML can be found on [Cross Validated](https://stats.stackexchange.com/a/544958).
:::
...
- **T-learner** - **t**wo models, one for each of the control and treatment groups respectively; predict the values as if all observations are 'treated' versus when all are 'control' using both models, and take the difference.
- **X-learner** - a more complicated modification to the T-learner also using a multi-step approach.


Some additional variants of these models exist, and they can be used in a variety of settings, and are very common in  uplift modeling. As elsewhere, the key idea is to use the model to predict the potential outcomes of the treatment levels to estimate the causal effect.
...
- Marginal structural models[^marginal]
- Instrumental variables and two-stage least squares[^iv]
- Propensity score matching/weighting
- Regression discontinuity design
- Mediation/moderation analysis[^semass]
- Meta-analysis[^semass]
- Bayesian networks
...
[^semass]: Mediation/moderation are special cases of SEM. SEM are more common in psychology and education, though less widely used these days in general.
...
In this exercise, we'll look at a simple example of a collider in the manner we did the confounder. First, run the available code to see what you get. Then, change it to the simulation approach we used for the confounder example, and change some of the relevant coefficients around.
