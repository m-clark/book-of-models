...
What everyone typically just calls "correlation" or Pearson's *r* is actually the *Pearson Product-Moment Correlation* -- that is a lot of words, though, so we will just go with correlation. As we mentioned earlier, we want to give you the choice to dive in as deep as you want. If you want just a little bit of background, you can read through the *Overview* section. If you want to see the easy way to use the model, you can work through the *Standard Function* section. If you want to see the method from start to finish, feel free to work through the *Roll Your Own* section. If all of that sounds like a lot of work and you'd rather look at pictures, check out the *Visualization* section.
...
Variables *x* and *y* are nothing but columns within data: maybe height and weight, years worked and salary, or vertical jump and broad jump distance. Just as a notation refresher, everything with a bar over it -- $\bar{x}$ and $\bar{y}$ -- indicates the mean of the variable. Everything with a subscripted *i* -- $x_i$ and $y_i$ -- indicates every observation of that variables. Said in real words, then, we are going to subtract the mean of *x* from every observation of *x*, subtract the mean of *y* from every observation of *y*, multiply them together, and then sum them to produce a single value. If you've taken a statistics class before, you might have seen this part of the formula when talking about *covariance* -- the joint variability of two variables. 
...
A great deal of the denominator might also look familiar. The individual pieces -- $\Sigma(x_i-\bar{x})^2$ and $\Sigma(y_i-\bar{y})^2$ -- are also used to compute *variance*. Taking the square root of variance gives us the *standard deviation*.  Using the standard deviation will help to mitigate the effects of *x* and *y* potentially being on different scales; in other words, we want to know how much two variables move together, even if the means might be very different. 
...
Let's generate those same values that we saw in that visualization:
...
Before we create our own function, let's see what R's `cor` function gives us:
...
my_cor <- function(x, y) {
  ...
  x_bar <- mean(x)
  y_bar <- mean(y)
  # Next, we can subtract x_bar from every value of x, 
  # and then do the same for every value of y and y_bar. 
  # This tells us how much each observation of x and y
  # differ from their respective means. We finish by
  # multiplying all of those values and then summing
  # to produce a single value:
  numerator <- sum((x - x_bar) * (y - y_bar))
  # Now we can move onto those standard deviations within the
  # denominator. We will still use x - x_bar and y - y_bar, 
  # but will square each of the values to make them positive
  # and sum them individually, multiply those two vectors, 
  # and then take the square root:
  denominator <- sqrt(sum((x - x_bar)^2) * sum((y - y_bar)^2))
  # We will finish by dividing the numerator by the denominator.
...
We can start by generating *x* and *y* values with `numpy`:
...
::: {.callout-note}
The `corrplot` package offers easy methods for visualizing correlation matrices. The `ggcor` package is a notable alternative, with more flexibility and arguably better aesthetics. The `corrr` package represents the newest wave and is worth using for its excellent correlation displays.
:::
