...
- **Covariance**: is the joint variability of two variables, i.e. how they vary together
...
This is where the denominator comes into play. To put it into real terms, we will subtract the mean of *x* from every observation of *x* (i.e. get the deviations of x), square those values, and sum those values to produce a single value. We will do the same for *y*, multiply those results, and then take the square root of that product. The denominator might also look familiar to some. The individual pieces- $\Sigma(x_i-\bar{x})^2$ and $\Sigma(y_i-\bar{y})^2$ - are computing the *variance* of x and y. Taking the square root of variance gives us the *standard deviation*.  So basically we're getting a combined standard deviation to scale the covariance. Using the standard deviation will mitigate the effects of *x* and *y* potentially being on different scales; in other words, we want to know how much two variables move together, even if the means and typical movement (deviation) might be very different. 
...
# Fiddle with the .5 and .75. The first can also be negative if you like!
...
```{r standard_cor}
#| eval: FALSE
# Results for R and Python will be slightly different due to different random number generators
cor(x, y)
np.corrcoef(x, y)
```
...
Since we already have a correlation matrix, we can use various means to find those patterns, which include visualizing the matrix itself, network graphs and others.  Her is one way to do it.
...
We can now visualize the correlation matrix.  Depending on how we c
```{python seaborn_demo}
#| echo: false
#| eval: false
