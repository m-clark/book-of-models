...
my_cor <- function(x, y) {
  ...
  numerator <- sum((x - x_bar) * (y - y_bar))
  # Now we can move onto those standard deviations within 
  # the denominator. We will still use x - x_bar and 
  # y - y_bar, but will square each of the values to make 
  # them positive and sum them individually, multiply those 
  # two vectors, and then take the square root:
  denominator <- sqrt(
    sum((x - x_bar)^2) * sum((y - y_bar)^2)
  )
