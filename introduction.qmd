...
You don't need to memorize this formula to interpret the correlation results, but knowing how those individual pieces work together only helps to deepen your understanding of the model -- we will take a look at the code later in the chapter. Diving into the internal workings of models also allows you to see how they connect to each other -- this provides a nice roadmap from chapter to chapter. 
...
- A brief dissection of the formula

- *When to use and how to interpret the model*

- Code

We will present the code in two ways:
...
We have a negative value, but this correlation is certainly nothing impressive -- we wouldn't expect impressive results with random sample! We already know the answer, so let's make sure that we can get the same answer by working through the formula.

Since we have our x and y vectors, we can compute the averages -- representing $\bar{x}$ and $\bar{y}$ -- for both:
...
Let's work through the numerator first. For the first term, $x_i-\bar{x}$, we can subtract `x_bar` from every value of `x`:

```{r}
x_minus_x_mean <- x - x_bar
```

This tells us how much each observation of `x` differs from the mean of `x`. 
...
While the long form certainly took more lines of code, we can see that breaking it down into smaller steps isn't difficult. For the sake of completeness, let's see how we would do the same thing with Python:
...
Now we can expand into the long form:
