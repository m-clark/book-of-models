...
Throughout this book, we will be presenting demonstrations in both R and Python, and you can use both or take your pick, but we want to leave that choice up to you. Our goal is to use them as a tool to help understand some big model ideas. Our default shows R first for statistical models, and Python for machine learning approaches and beyond[^rpytabs], as we feel their relative strengths are in those areas, and for a  balanced focus. But either language can be used well for any modeling task in this book.

[^rpytabs]: The web version uses tabs, and clicking on the R or Python tab will change all tabs to that selection so you can stick with your preferred language.
