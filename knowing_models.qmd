...
p2 = p_dat |>
    ...
    stat_ecdf(geom = 'borderline', aes(color = name), linewidth = 1) +
...
![Observed vs. Predicted Ratings](img/knowing-pred_vs_obs.svg)
:::
...
:::{.content-visible when-format='pdf'}
![ROC curve and AUC value](img/knowing-roc_auc.svg)
:::
...
:::{.content-visible when-format='pdf'}
![Residuals vs. Fitted and PP Check plots for our regression model](img/knowing-model-select-residuals.svg)
:::
...
:::{.content-visible when-format='pdf'}
![MSE Loss over 25 epochs in an MLP model](img/knowing-model-select-training-loss.svg)
:::
...
:::{.content-visible when-format='pdf'}
![Posterior Predictive Check for a regression model](img/knowing-model-select-pp-check.svg)
:::
