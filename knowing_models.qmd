...
ev = eigen(vcov(model_reviews_shap))
sqrt(max(ev$values) / min(ev$values))
car::vif(model_reviews_shap)
```
...
# inspect if desired
# model_reviews_shap.summary(slim = True)
```

:::


With our model in place let's look at the SHAP values for our model.  We'll start with a single feature value/observation. Here we'll use the first observation where the release year is 2020, age of reviewer is 30, a movie length of 100 minutes. To aid our understanding, we calculate the shap value related to word count at that observation by hand, and using a package.
...
avg_pred = mean(predict(model_reviews_shap))
...
pred_year_2020 = predict(
    model_reviews_shap,
    newdata = df_reviews |> mutate(release_year = 2020) 
)

pred_length_100 = predict(
    model_reviews_shap,
    newdata = df_reviews |> mutate(length_minutes = 100) 
)


# then we can calculate the shap values
shap_value_ours = tibble(
    shap_age    = mean(pred_age_30) - avg_pred,
    shap_year   = mean(pred_year_2020) - avg_pred,
    shap_length = mean(pred_length_100) - avg_pred
)

# we can also use the DALEX package to do this for us
explainer = DALEX::explain(model_reviews_shap, verbose = FALSE)
...
# first we need to get the average prediction
avg_pred = model_reviews_shap.predict(df_reviews).mean()

# then we need to get the prediction for the feature value of interest
# for all observations, and average them

pred_age_30 = model_reviews_shap.predict(
    df_reviews.assign(
        age = 30
    )
)

pred_year_2020 = model_reviews_shap.predict(
    df_reviews.assign(
        release_year = 2020
    )
)

pred_length_100 = model_reviews_shap.predict(
    df_reviews.assign(
        length_minutes = 100
    )
)
...
shap_value_ours = pd.DataFrame({
    ...
    'release_year': pred_year_2020.mean() - avg_pred,
    'length_minutes': pred_length_100.mean() - avg_pred
...
tibble(
    ...
    dalex = shap_value_package['age', 'contribution']
) |>
    gt(decimals = 3)
```
